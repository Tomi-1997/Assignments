...
GUIShape,65280,true,2,Rect2D,0.1,0.2,0.5,0.45
GUIShape,65535,false,3,Rect2D,0.1,0.2,0.17,0.77
GUIShape,8421504,false,6,Rect2D,-0.04000000000000001,0.06,0.8,0.775
Enter a sorting method (a number in [0-6]): -1
Done! bye bye..
 * 
...
public class Test_Ex4 {
	...
	public static void test1() {
		...
		GUI_Shape s2 = new GUIShape(c1,true, Color.magenta.darker().darker(), tag++);
		GUI_Shape s3 = new GUIShape(r1,true, Color.green.darker().darker(), tag++);
		GUI_Shape s4 = new GUIShape(r2,false, Color.cyan.darker(), tag++);
		GUI_Shape s5 = new GUIShape(t1,false, Color.orange, tag++);
		GUI_Shape s6 = new GUIShape(seg1,true, Color.pink, tag++);
		
		/* */
		Point2D _p = new Point2D(0.680625,0.64203125);
		GUI_Shape s_p = new GUIShape(_p, true, Color.black, tag++);
		sc.add(s_p);
		/* */
		sc.add(s1);
		sc.add(s6);
...
GUIShape,8421504,false,6,Rect2D,-0.04000000000000001,0.06,0.8,0.77
	...
	public static void test2() {
		Ex4 win = new Ex4();
		GUI_Shape_Collection sc = win.getShape_Collection();
		sc.load("test_save.txt"); //seg tri cir
		...
		while(flag!=-1) { 
			...
			System.out.println();
		Rect2D bb = sc.getBoundingBox();
		GUI_Shape s7 = new GUIShape(bb,false, Color.PINK, 2);
		sc.add(s7);
		Point2D min = bb.getPoints()[0], max = bb.getPoints()[1];
		double m0 = Math.min(min.x(), min.y());
		double m1 = Math.max(max.x(), max.y());
